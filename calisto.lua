...
package.preload["lgi.override.Gdk"] = function()
    return {}
end
package.preload["lgi.override.Gtk"] = function()
    return {}
end
...
function import(name) -- intentionally global
    ...
    if not f then
        error("import: cannot open '" .. path .. "'", 2)
    end
    local src = f:read("a")
    f:close()
...
app.on_activate = function()
    ...
    -- Auto size - let the window size naturally to fit content
